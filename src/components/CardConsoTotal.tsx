...
// Fonction pour formater les valeurs avec les bonnes unités selon la magnitude
const formatValue = (value: Big, baseUnit: string, type: string): { formattedValue: string, displayUnit: string } => {
    if (type !== "electricity" || baseUnit !== "kWh") {
        // Si ce n'est pas de l'électricité ou l'unité n'est pas kWh, on retourne la valeur telle quelle
        return { 
...
export const CardConsoTotal = ({ title, currentValue, previousValue, unit, type }: CardConsoTotalProps): ReactElement => {
    ...
    const formattedCurrentValue = hasData ? formatValue(currentValue, unit, type) : { formattedValue: "N/A", displayUnit: unit };
